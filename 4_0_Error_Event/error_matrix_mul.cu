...
int main()
{
    ...
    CHECK(cudaMemcpy( gpu_a, cpu_a, memsize + 10000, cudaMemcpyHostToDevice));  // check error
...
// 另外还可以使用 CUDA-MEMCHECK 检查内存错误, 参见 https://docs.nvidia.com/cuda/cuda-memcheck
// 建简易使用: cuda-memcheck ./a.out